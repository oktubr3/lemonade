/**
 * Lemonade Pass Manager - Background Service Worker
...
async function getPendingCredential() {
    const data = await chrome.storage.session.get(['pendingCredential', 'pendingExistingEntry', 'pendingTimestamp']);
    if (!data.pendingCredential || !data.pendingTimestamp) return { success: true, data: null };
    if (Date.now() - data.pendingTimestamp > 30000) {
        await chrome.storage.session.remove(['pendingCredential', 'pendingExistingEntry', 'pendingTimestamp']);
...
async function getAuthState() {
    const stored = await chrome.storage.local.get(['user', 'authToken', 'tokenExpiry']);
...
async function loginWithGoogle() {
    try {
        // Build Google OAuth URL - remove trailing slash as Google Console doesn't accept it
...
async function logout() {
    try {
        await chrome.storage.local.remove(['user', 'authToken', 'tokenExpiry', 'refreshToken', 'credentials']);
        return { success: true };
    } catch (error) {
...
async function getValidToken() {
    const stored = await chrome.storage.local.get(['authToken', 'tokenExpiry', 'refreshToken']);
...
async function getAllCredentials() {
    try {
        ...
        const cached = await chrome.storage.local.get(['credentials']);
