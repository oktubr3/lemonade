...
function getIconForUrl(url) {
    ...
    // Parse hostname rather than substring-matching the whole URL.
    // Suffix-match prevents attacker-controlled paths like
    // https://evil.x.com.attacker.com/ from matching brand emojis.
    let host;
    try {
        const urlToTest = url.match(/^https?:\/\//) ? url : `https://${url}`;
        host = new URL(urlToTest).hostname.toLowerCase();
    } catch {
        return '🔑';
    }
    ...
    if (matchesDomain('google.com') || matchesDomain('googleapis.com')) return '🔵';
    if (matchesDomain('github.com')) return '🐙';
    if (matchesDomain('facebook.com') || matchesDomain('meta.com')) return '📘';
    if (matchesDomain('twitter.com') || matchesDomain('x.com')) return '🐦';
    if (matchesDomain('amazon.com')) return '📦';
    if (matchesDomain('apple.com')) return '🍎';
    if (matchesDomain('microsoft.com')) return '🪟';
    if (matchesDomain('netflix.com')) return '🎬';
    if (matchesDomain('spotify.com')) return '🎵';
    if (matchesDomain('linkedin.com')) return '💼';
...
async function handleCapturedCredential(capturedData) {
    try {
        ...
        const { dismissedDomains = [] } = await browser.storage.local.get('dismissedDomains');
        if (dismissedDomains.includes(capturedData.url)) return;

        // Check if user is logged in
        const authResponse = await browser.runtime.sendMessage({ type: 'GET_AUTH_STATE' });
        if (!authResponse || !authResponse.isLoggedIn) {
            return; // Silently skip — don't interrupt the user
        }
        ...
        try {
            const credResponse = await browser.runtime.sendMessage({
                type: 'GET_CREDENTIALS',
        ...
        if (existingEntry) {
            try {
                const pwResponse = await browser.runtime.sendMessage({
                    type: 'GET_DECRYPTED_PASSWORD',
...
async function checkPendingCredential() {
    try {
        const response = await browser.runtime.sendMessage({ type: 'GET_PENDING_CREDENTIAL' });
    ...
    neverBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            if (!dismissedDomains.includes(capturedData.url)) {
                ...
                await browser.storage.local.set({ dismissedDomains });
            }
        } catch (e) {
            // Silently fail
        }
